import {
    ISelect, IDataBaseOption, IRemove, IInsert,
    IUpdate, IWebWorkerResult, IWebWorkerRequest, IConfig, ISelectJoin, ITranscationQry
} from "../interfaces";
import { IError, Error_Type, ErrorHelper } from "../error_helper";
import { DataBase } from "../Model/DataBase";
import { getDbVersion, getDbSchema } from "../HelperLogic";
import { db_version, db_status, file_name, log, enable_log } from "../CommonLogic";
import { DbHelper } from "../Model/db_helper";
import { TableHelper } from "../Model/table_helper";
import { Connection_Status } from "../enums";
import * as Select from '../web_worker/Select/main';
import * as Count from '../web_worker/Count/main';
import * as Remove from '../web_worker/Remove/main';
import * as Update from '../web_worker/Update/main';
import * as Insert from '../web_worker/Insert/main';
import { DropDb } from "./DropDbLogic";
import { Transaction } from "./TransactionLogic";
import { Clear } from "./ClearLogic";
import { CreateDb } from "./CreateDbLogic";
import { OpenDb } from "./OpenDbLogic";
import { BulkInsert } from "./BulkInsertLogic";
...
export class QueryExecutor {
        ...
        for (var prop in config) {
            switch (prop) {
                ...
                case 'OnDbDroppedByBrowser': eval("on_db_dropped_by_browser=" + config.OnDbDroppedByBrowser);
                    break;
                default:
                    var err = new ErrorHelper(Error_Type.InvalidConfig, { Config: prop });
                    err.logError();
            }
        }
    }
    ...
    private openDb(dbName, onSuccess: () => void, onError: (err: IError) => void) {
        getDbVersion(dbName, function (dbVersion) {
            if (dbVersion !== 0) {
                ...
                var err = new ErrorHelper(Error_Type.DbNotExist, { DbName: dbName });
                err.logError();
                onError(err.get());
            }
        });
    ...
    private createDb(
        ...
        var processCreateDb = function () {
            ...
            // create meta data
            var db_helper = new DbHelper(active_db);
            db_helper.createMetaData(function (tablesMetaData: TableHelper[]) {
        ...
        if (dataBase == null) {
            ...
            getDbVersion(dataBase.Name, function (version) {
                ...
                active_db = new DataBase(dataBase);
                processCreateDb();
            });
        }
