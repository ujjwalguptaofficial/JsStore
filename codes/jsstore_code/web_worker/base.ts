import { BaseHelper } from "./BaseHelperLogic";
import { IError, Error_Type, ErrorHelper } from "../error_helper";
import { WhereChecker } from "./WhereChecker";
import { logError, getObjectFirstKey } from "../CommonLogic";
import { Column } from "../Model/Column";
import { Occurence, WhereQryOption } from "../enums";
...
export class Base extends BaseHelper {
    ...
    protected onExceptionOccured(ex: DOMException, info) {
        switch (ex.name) {
            case 'NotFoundError':
                var error = new ErrorHelper(Error_Type.TableNotExist, info);
    ...
    protected goToWhereLogic = function () {
        ...
        if (this._objectStore.indexNames.contains(column_name)) {
            ...
            var column: Column = this.getColumnInfo(column_name),
                error;
            if (column == null) {
                error = new ErrorHelper(Error_Type.ColumnNotExist, { ColumnName: column_name }).get();
            }
            else {
                error = new ErrorHelper(Error_Type.EnableSearchOff, { ColumnName: column_name }).get();
            }
