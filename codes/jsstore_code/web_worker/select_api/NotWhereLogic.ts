import { BaseSelect } from "./BaseSelectLogic";
import { Error_Type, ErrorHelper } from "../../error_helper";
    ...
    protected executeWhereUndefinedLogic() {
        if (this._query.Order && this._query.Order.By) {
            if (this._objectStore.indexNames.contains(this._query.Order.By)) {
                ...
                var error = new ErrorHelper(Error_Type.ColumnNotExist, { ColumnName: this._query.Order.By });
                error.throw();
            }
        }
        else {
