...
(function (JsStore) {
    ...
    (function (Business) {
        var CreateDbLogic = (function () {
            function CreateDbLogic(dbVersion, onSuccess, onError) {
    ...
    (function (Business) {
        var MainLogic = (function () {
            function MainLogic(onSuccess) {
                ...
                this.insert = function (query, isReturn, onSuccess, onError) {
                    if (!Array.isArray(query.Values)) {
                        throw "Value should be array :- supplied value is not array";
                    }
                    else if (query.Values.length > 0) {
                        var ObjInsert = new Business.InsertLogic(query.Into, query.Values, isReturn, onSuccess, onError);
                    }
                    else {
                        if (onError != null) {
                ...
                this.createDb = function (dataBase, onSuccess, onError) {
                    ...
                    new Business.CreateDbLogic(DbVersion, onSuccess, onError);
                };
...
(function (JsStore) {
    ...
    (function (WebWorkerStatus) {
        WebWorkerStatus["Registering"] = "registering";
    ...
    var Instance = (function () {
        function Instance() {
            ...
            this.prcoessExecutionOfCode = function (request) {
                ...
                if (this.WorkerStatus == WebWorkerStatus.Registered && this.QueryRequests.length == 1) {
                    console.log("request accepted:" + this.QueryRequests[0]);
                    this.executeCode();
                }
            };
            this.executeCode = function () {
                if (this.QueryRequests.length > 0) {
                    var Request = {
                        Name: this.QueryRequests[0].Name,
                        Query: this.QueryRequests[0].Query
                    };
                    if (this.WorkerInstance) {
                        this.executeCodeUsingWorker(Request);
                    }
                    else {
                        this.executeCodeDirect(this.QueryRequests[0]);
                    }
                }
            };
            this.executeCodeDirect = function (request) {
                var That = this;
                new JsStore.Business.MainLogic(function (results) {
            ...
            this.processFinishedRequest = function (message) {
                console.log("request finished:" + message);
            ...
            this.createWorker = function () {
                var That = this;
                new JsStore.WebWorker.MainLogic().getWorkerUrl(function (url) {
                    That.WorkerInstance = new Worker(url);
                    That.WorkerInstance.onmessage = function (msg) {
                        That.processFinishedRequest(msg.data);
                    };
                    That.executeCode();
                }, function () {
        ...
        var MainLogic = (function () {
            function MainLogic() {
                ...
                this.getWorkerEventsinString = function () {
                    var WorkerEventsInString = "\n;self.onmessage = function (e) {\n                    debugger;\n                var Request = e.data, IndexDbObject = new JsStore.Business.MainLogic();\n                IndexDbObject.checkConnectionAndExecuteLogic(Request);\n                };";
                    return WorkerEventsInString;
                };
                this.registerWorkerEvents = function () {
                    console.log('worker events registered');
                    var That = this;
                    self.onmessage = function (e) {
                        var Request = e.data, IndexDbObject = new JsStore.Business.MainLogic();
                        IndexDbObject.checkConnectionAndExecuteLogic(Request);
                ...
                this.getStringOfModels = function () {
                    var String = "";
                    for (var model in JsStore.Model) {
                        var TmpString = "var Model; \n                        (function (Model) { \n                        var " + model + " = (function () { \n                            " + JsStore.Model[model].toString() + " \n                            return " + model + " \n                        }());\n                        Business." + model + " = " + model + " ; \n                        })(Business || (Business = {}));";
                        String += TmpString;
                    }
                    return String;
                };
                this.getStringOfBusiness = function () {
                    var String = "";
                    for (var business in JsStore.Business) {
                        var InheritedClass = this.getInheritedClassName(business), TmpString;
                        if (InheritedClass) {
                            TmpString = "var Business;" +
                                "(function (Business) {" +
                                "var " + business + "= (function () {" +
                                JsStore.Business[business].toString() + "return " + business +
                                "}(Business." + InheritedClass + "));" +
            ...
            MainLogic.prototype.getScriptUrl = function (fileName) {
                var ScriptUrl = "";
            ...
            MainLogic.prototype.getInheritedClassName = function (className) {
                return JsStore.Business[className].prototype.constructor.__proto__.name;
            };
            return MainLogic;
        }());
        WebWorker.MainLogic = MainLogic;
